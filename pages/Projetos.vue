...
const { data: repos } = useLazyAsyncData('user', () => $fetch('https://api.github.com/users/alexislopes/repos?per_page=100&sort=updated'))

const projetos = computed(() => {
  return repos.value?.filter(repo => repo.topics.includes('portifolio'))
})
