<template>
  <div :class="cn('relative inline-block', props.class)">
    <!-- Trigger -->
    <!-- <NuxtLink :to="url" :class="cn('text-black dark:text-white', props.linkClass)" @mousemove="handleMouseMove"
      @mouseenter="showPreview" @mouseleave="hidePreview">
      <slot />
    </NuxtLink> -->

    <!-- Preview -->
    <div ref="preview" class="pointer-events-none absolute z-50" :style="previewStyle">
      <div class="overflow-hidden rounded-xl shadow-xl" :class="[popClass, { 'transform-gpu': !props.isStatic }]">
        <div class="block rounded-xl border-2 border-transparent bg-white p-1 shadow-lg dark:bg-gray-900">
          <img :src="previewSrc" :style="imageStyle" class="rounded-md" alt="preview" @load="handleImageLoad" />
        </div>
      </div>
    </div>
  </div>
